...
lab_names <- lubridate:::.shift_wday_names(lubridate:::.get_locale_regs("en_GB.UTF-8")$wday_names$abr, week_start = 1)
...
```{r users-plots}
pal <- RColorBrewer::brewer.pal(name = "Paired", n = 12)
(bioc_users_plot + 
...
bioc_author_reviewer_comments <- bioc_by_issue %>% 
  ...
  labs(size = "Issues", x = "Authors", title = "Reviewers", y = element_blank())
...
bioc_auth_other_comments <- bioc_by_issue %>% 
  ...
  labs(size = "Issues", x = "Authors", title = "Other", y = element_blank())
...
ros_author_editor_comments <- ros_by_issue %>% 
  ...
  labs(size = "Issues", x = "Authors", title = "Editors", y = element_blank())
...
ros_auth_other_comments <- ros_by_issue %>% 
  ...
  labs(size = "Issues", x = "Authors", title = "Reviewers & other", y = element_blank())
...
.center[Labels are used to indicate progress on the submission.]
...
success_dates <- ap0 %>% 
  ...
  filter(Accepted) %>% 
  ggplot() + 
  geom_smooth(aes(submission, perc, col = new, linetype = new), span = 0.5) +
  scale_y_continuous(expand = expansion(), labels = scales::percent) +
  scale_x_datetime(date_labels = "%y/%m", date_breaks = "1 month", 
                   expand = expansion()) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = c(0.2, 0.4)) +
  labs(col = "Submission", title = "Acceptance rate", x = element_blank(), linetype = "Submission", y = element_blank())
success_submissions + success_dates
```

.center[High approval rates!!]
...
Blog posts for [Bioconductor](https://llrs.dev/2020/07/bioconductor-submissions-reviews/), [rOpenSci](https://llrs.dev/2020/09/ropensci-submissions/), [CRAN](https://llrs.dev/2021/01/cran-review/). 
