...
names(l) <- unique(rsubm$package)
f <- function(start, package, li) {
  dates <- li[[package]]
...
ap <- rsubm %>% 
  ...
  summarize(start = min(start), end = max(end), 
            newbies = ifelse(sum(newbies) > 0, TRUE, FALSE)) %>% 
  ungroup() %>% 
  rowwise() %>% 
  mutate(accepted = f(start, package, l)) %>% 
  ungroup() %>% 
  mutate(tdiff = difftime(accepted, start, units = "hours")) 
ap %>% 
