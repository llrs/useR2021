...
df <- data.frame(Objective = objective, 
...
cran_dweek <- cran_times %>% 
  ...
  labs(x = "Week dat", y = element_blank(), col = "Folder", 
       linetype = "Folder",
...
.center[Some users help with many reviews.]
...
```{r cran_success}
approval_dates <- function(start, package, li) {
  dates <- li[[package]]
  dates <- dates[!is.na(dates)] # Too old packages don't have date
  if (is.logical(dates)) {
    return(NA)
  }
  n <- near(dates, start, tol = 23)
  any_date <- dates > start | n
  if (!any(any_date)) {
    return(NA)
  }
  min(dates[any_date])
...
ap0 <- rsubm %>% 
  group_by(package, submission_n) %>% 
  rowwise() %>% 
  mutate(accepted = approval_dates(start, package, cran_dates)) %>% 
  distinct(package, submission_n, accepted)
...
Bioconductor: 1 month,  rOpenSci: around 2 months
