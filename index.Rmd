...
meta() %>%
  ...
  meta_social(
    ...
    image = "https://user2021.llrs.dev/index_files/figure-html/title-plot-1.png",
    image_alt = paste(
      "Image with submissions to CRAN for Reviewing packages; how does it work?", 
      "Horizontal axis the date, vertical axis the submissions on CRAN each line is a package on the submission queue. It shows a continuous increase on packages submitted (except some pauses).", 
...
```{r cran-holidays-zoom, fig.alt="A zoom from the previous plot to only show the pacakges on queue after the holidays. The spike on pretest package after holidays is clearly seen (reaches ~140 pacakges), followed by a sustained high number of packages on newbies (around ~70 pacakges) until middle February. At the beginning of April another spike of pretest pagkaes but  newbies remain at 25 pacakges and pretest even lower."}
...
```{r cran-day-month, fig.alt="Two plots with a loess estimation of the number of pacakges on the folders newbies and pretest. On the left by day of month: Newbies has some dip at the beginning of the month and around day 20-29 while pretests is constant around 50 packages each day. On the right by day of week: many pacakges at the beginning of the week and fewer on the weekend. Pretest packages fall from 50 to around 30, while newbies drops from 80 to 70."}
cran_times <- cran %>% 
...
```{r cran-review, fig.alt="Histogram of time that a submission is on the queue. One big histogram from 0 to over 2000 hours, where most there are below 500h and decay in logarithmic pattern. Above it a zoom on the first week, split by 24h till 168h (1 week). Most submissions are less than 24h on the queue."}
...
```{r cran-submission-time, fig.alt="A plot with the loess estimation of hours for submission one line by if the pacakge is new another if it is an update."}
subm_time <- rsubm %>% 
  group_by(package, submission_n) %>% 
...
bioc_author_reviewer_comments <- bioc_by_issue %>% 
  ...
  geom_count(aes(author_comments, reviewer_comments), shape = 17, 
             col = "#87b13f") +
  labs(size = "Bioconductor", x = "Authors", title = "Reviewers", y = element_blank())
...
ros_author_editor_comments <- ros_by_issue %>% 
  ...
  labs(size = "rOpenSci", x = "Authors", title = "Editors", y = element_blank())
...
```{r cran_success, fig.alt="On the left a bar plot with packages submissions on the x axis and on the vertical axis the number of pacakges. The bars are colored by if they are accepted or not. It is also split by new packages and update pacakges. More new pacakges are not accepted on the first try than updates, but on resubmissions they are accepted. The plot on the right shows the acceptance rate for the range of dates from 2020/09 to 2021/06. Two lines with one for new submissions which shows a consistend rate around 81% and package updates is between 85% and 95% (that is until the time series get to close for the review to be finished)."}
...
Thank also the package authors (mainly tidyverse).
